...
int ha_mroonga::generic_store_bulk_json(Field *field, grn_obj *buf, int nth_column)
{
  ...
  if (!mrn::grn::is_vector_column(column)) {
    ...
    grn_obj_reinit(ctx, buf, GRN_DB_TEXT, GRN_OBJ_VECTOR);
    if (wrapper.depth() != 2) {
    ...
    if (mrn::grn::is_weight_column(column)) {
      ...
      for (Json_wrapper_object_iterator iter= wrapper.object_iterator();
        ...
        if (value.type() == Json_dom::J_INT) {
          grn_vector_add_element(ctx, buf, key_data, key_length, value.get_int(), GRN_DB_TEXT);
        } else {
          error = generic_store_bulk_json_error(json,
            "weight vector column's value should be integer: <%.*s>");
          goto exit;
        }
      }
    } else {
      if (wrapper.type() != Json_dom::J_ARRAY) {
        error = generic_store_bulk_json_error(json,
        ...
        for (int i = 0; i < array_length; i++) {
          ...
          if (range->header.flags & GRN_OBJ_KEY_VAR_SIZE &&
              element.type() == Json_dom::J_STRING) {
            grn_vector_add_element(ctx, buf, element.get_data(), element.get_data_length(),
                                   0, GRN_DB_TEXT);
          } else if (!mrn::grn::is_table(range) &&
                     range->header.flags & GRN_OBJ_KEY_INT &&
                     element.type() == Json_dom::J_INT) {
            if (i == 0) {
              grn_obj_reinit(ctx, buf, range_id, GRN_OBJ_VECTOR);
            }
            longlong int_value = element.get_int();
            grn_vector_add_element(ctx, buf, (const char *)&int_value,
                                   grn_type_size(ctx, range), 0, range_id);
          } else {
            error = generic_store_bulk_json_error(json,
              "vector column's value support string or integer only: <%.*s>");
            break;
          }
        }
      }
    ...
    for (int i = 0; i < n_ids; i++) {
      ...
      if (range->header.flags & GRN_OBJ_KEY_VAR_SIZE) {
        ...
        if (mrn::grn::is_table(range)) {
          grn_id id = grn_uvector_get_element(ctx, value, i, &weight);
