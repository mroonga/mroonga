...
static inline void mrn_generic_ft_ensure_searched(
  st_mrn_ft_info *info,
  bool need_fast_order_limit_check)
{
  ...
  {
    longlong escalation_threshold = THDVAR(info->mroonga->current_thread(),
                                           match_escalation_threshold);
    mrn::MatchEscalationThresholdScope scope(info->ctx,
      ...
      if (db->is_broken_table(table_name, table_name_size)) {
        ...
        if (!error) {
          ...
          if (!share->disable_keys) {
            ...
            // other threads.
            error = wrapper_disable_indexes_mroonga(MRN_HANDLER_DISABLE_INDEXES_ALL_ARGS);
            if (!error) {
              error = wrapper_enable_indexes_mroonga(MRN_HANDLER_ENABLE_INDEXES_ALL_ARGS);
            }
          }
        }
        GRN_LOG(ctx, GRN_LOG_NOTICE,
...
int ha_mroonga::storage_write_row_multiple_column_indexes(mrn_write_row_buf_t buf,
                                                          grn_id record_id)
{
  MRN_DBUG_ENTER_METHOD();
                                             ...
                                             page_range *pages)
{
  ...
  if (mrn_is_geo_key(key_info)) {
    ...
    MRN_SET_WRAP_SHARE_KEY(share, table->s);
    MRN_SET_WRAP_TABLE_KEY(this, table);
    row_count = wrap_handler->records_in_range(key_nr,
                                               range_min,
                                               range_max,
                                               pages);
    MRN_SET_BASE_SHARE_KEY(share, table->s);
...
int ha_mroonga::generic_ft_init()
{
  ...
  st_mrn_ft_info *mrn_ft_info =
    reinterpret_cast<st_mrn_ft_info *>(ft_handler);
...
st_mrn_ft_info *ha_mroonga::generic_ft_init_ext_select(uint flags,
                                                       uint key_nr,
                                                       String *key)
{
  MRN_DBUG_ENTER_METHOD();
                                      ...
                                      const char *field_name)
{
  ...
  bool is_foreign_key_field_ =
    is_foreign_key_field(table_name,
                         field_name,
                         strlen(field_name));
...
#ifdef MRN_ENABLE_WRAPPER_MODE
int ha_mroonga::wrapper_disable_indexes_mroonga(MRN_HANDLER_DISABLE_INDEXES_PARAMETERS)
{
  int error = 0;
  MRN_DBUG_ENTER_METHOD();
...
int ha_mroonga::wrapper_disable_indexes(MRN_HANDLER_DISABLE_INDEXES_PARAMETERS)
{
  ...
  MRN_SET_WRAP_SHARE_KEY(share, table->s);
  MRN_SET_WRAP_TABLE_KEY(this, table);
  error = wrap_handler->ha_disable_indexes(MRN_HANDLER_DISABLE_INDEXES_FORWARD_ARGS);
  MRN_SET_BASE_SHARE_KEY(share, table->s);
  MRN_SET_BASE_TABLE_KEY(this, table);
  ...
  if (!error) {
    error = wrapper_disable_indexes_mroonga(MRN_HANDLER_DISABLE_INDEXES_FORWARD_ARGS);
  }
  DBUG_RETURN(error);
}
#endif
...
#ifdef MRN_ENABLE_WRAPPER_MODE
int ha_mroonga::wrapper_enable_indexes_mroonga(MRN_HANDLER_ENABLE_INDEXES_PARAMETERS)
{
  int error = 0;
  MRN_DBUG_ENTER_METHOD();
...
int ha_mroonga::wrapper_enable_indexes(MRN_HANDLER_ENABLE_INDEXES_PARAMETERS)
{
  ...
  int mroonga_error = wrapper_enable_indexes_mroonga(MRN_HANDLER_ENABLE_INDEXES_FORWARD_ARGS);

  MRN_SET_WRAP_SHARE_KEY(share, table->s);
  MRN_SET_WRAP_TABLE_KEY(this, table);
  error = wrap_handler->ha_enable_indexes(MRN_HANDLER_ENABLE_INDEXES_FORWARD_ARGS);
  MRN_SET_BASE_SHARE_KEY(share, table->s);
  MRN_SET_BASE_TABLE_KEY(this, table);
  if (error == HA_ERR_WRONG_COMMAND) {
    error = mroonga_error;
  ...
  Alter_inplace_info *ha_alter_info)
{
  ...
  if (ha_alter_info->handler_flags & MRN_ALTER_INPLACE_INFO_ALTER_FLAG(RENAME))
  {
    DBUG_RETURN(HA_ALTER_INPLACE_NOT_SUPPORTED);
  }
