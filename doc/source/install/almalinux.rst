...
.. _almalinux-8-oracle-8-4:

AlmaLinux 8 (with the Oracle MySQL 8.4 package)
-----------------------------------------------

You can use Oracle's MySQL packages version 8.4 on AlmaLinux 8 since
...
.. code-block:: console
   ...
   $ sudo dnf install -y  https://packages.apache.org/artifactory/arrow/almalinux/8/apache-arrow-release-latest.rpm
   $ sudo dnf install -y https://packages.groonga.org/almalinux/8/groonga-release-latest.noarch.rpm
   $ sudo dnf install -y https://repo.mysql.com/mysql84-community-release-el8.rpm
   $ sudo dnf install -y --enablerepo=epel,powertools mysql-community-8.4-mroonga
   ($ sudo systemctl start mysqld)
   ($ tmp_password=$(sudo grep 'A temporary password' /var/log/mysqld.log | sed -e 's/^.*: //'))
   ($ sudo mysqladmin -u root --password="${tmp_password}" password)
...
.. _almalinux-8-mariadb-11-8:

AlmaLinux 8 (with MariaDB 11.8 package)
---------------------------------------

You can use MariaDB's MariaDB packages version 11.8 on AlmaLinux 8 since
Mroonga 15.21 release.
...
.. code-block:: console
   ...
   $ sudo dnf install -y --enablerepo=powertools mariadb-server
   $ sudo systemctl start mariadb
   $ sudo dnf install -y --enablerepo=powertools mariadb-11.8-mroonga
   ($ sudo mariadb-admin -u root password 'new-password')
...
.. _almalinux-10-oracle-8-4:

AlmaLinux 10 (with the Oracle MySQL 8.4 package)
------------------------------------------------

You can use Oracle's MySQL packages version 8.4 on AlmaLinux 10 since
Mroonga 15.18 release.
...
.. code-block:: console
   ...
   $ sudo dnf install --disablerepo=AppStream -y --enablerepo=epel mysql-community-8.4-mroonga
   ($ sudo systemctl start mysqld)
   ($ tmp_password=$(sudo grep 'A temporary password' /var/log/mysqld.log | sed -e 's/^.*: //'))
   ($ sudo mysqladmin -u root --password="${tmp_password}" password)
