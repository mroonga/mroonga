...
#  define KEY_NAME_EQUAL_KEY(key1, key2)                                       \
    ((key1)->name.length == (key2)->name.length &&                             \
     strncmp((key1)->name.str, (key2)->name.str, (key1)->name.length) == 0)
#  define KEY_NAME_FORMAT "%.*s"
#  define KEY_NAME_FORMAT_VALUE(key)                                           \
    static_cast<int>((key)->name.length), (key)->name.str
#else
...
#  define FIELD_NAME(field) (field)->field_name.str, (field)->field_name.length
...
#else
#  define FIELD_NAME(field)             (field)->field_name, strlen((field)->field_name)
#  define FIELD_NAME_EQUAL(field, name) (strcmp((field)->field_name, name) == 0)
#  define FIELD_NAME_EQUAL_STRING(field, string)                               \
...
#  define MRN_KEY_PART_SPEC_FIELD_NAME_VALUE(key_part_spec) (key_part_spec)
...
typedef malloc_unordered_map<std::string,
...
#ifdef MRN_MARIADB_P
#  define MRN_QUERY_BLOCK_GET_WHERE_COND(query_block)  ((query_block)->where)
#  define MRN_QUERY_BLOCK_GET_HAVING_COND(query_block) ((query_block)->having)
...
#  define MRN_QUERY_BLOCK_GET_FIRST_VISIBLE_FIELD(query_block)                 \
    (static_cast<Item*>(                                                       \
      MRN_QUERY_BLOCK_GET_FIELDS_LIST(query_block).first_node()->info))
#endif
...
#else
#  define MRN_QUERY_BLOCK_HAS_LIMIT(query_block) (query_block->explicit_limit)
#endif
...
#else
#  define MRN_QUERY_BLOCK_SELECT_LIMIT(query_block) (query_block->select_limit)
#  define MRN_QUERY_BLOCK_OFFSET_LIMIT(query_block) (query_block->offset_limit)
#endif
...
#ifdef MRN_MARIADB_P
#  define mrn_is_directory_separator(c) (c == FN_LIBCHAR || c == FN_LIBCHAR2)
#else
#  define mrn_is_directory_separator(c) is_directory_separator((c))
#endif
...
#  define ITEM_SUM_GET_NEST_LEVEL(sum_item)                                    \
    (sum_item)->base_query_block->nest_level
#  define ITEM_SUM_GET_AGGR_LEVEL(sum_item)                                    \
    (sum_item)->aggr_query_block->nest_level
...
#define MRN_ALLOCATE_VARIABLE_LENGTH_ARRAYS(type,                              \
                                            variable_name,                     \
                                            variable_size)                     \
  type* variable_name =                                                        \
    (type*)mrn_my_malloc(sizeof(type) * (variable_size), MYF(MY_WME))
#define MRN_FREE_VARIABLE_LENGTH_ARRAYS(variable_name) my_free(variable_name)
...
#ifdef MRN_MARIADB_P
#  define MRN_GENERATED_COLUMNS_FIELD_IS_VIRTUAL(field) (!field->stored_in_db())
...
#else
#  define MRN_TABLE_LIST_TABLE_NAME_DATA(table_list) (table_list)->table_name
#  define MRN_TABLE_LIST_TABLE_NAME_LENGTH(table_list)                         \
    (table_list)->table_name_length
#endif

#if !defined(MRN_MARIADB_P) && MYSQL_VERSION_ID >= 80024
#  define MRN_TABLE_LIST_QUERY_BLOCK(table_list) (table_list)->query_block
#else
#  define MRN_TABLE_LIST_QUERY_BLOCK(table_list) (table_list)->select_lex
#endif
...
#  define MRN_KEY_SPEC_LIST_EACH_BEGIN(spec_list, spec)                        \
    do {                                                                       \
...
#if MYSQL_VERSION_ID >= 80011 && !defined(MRN_MARIADB_P)
#  define MRN_KEY_PART_SPEC_LIST_N_ELEMENTS(spec_list) spec_list.size()
#  define MRN_KEY_PART_SPEC_LIST_EACH_BEGIN(spec_list, spec)                   \
    do {                                                                       \
      for (size_t spec_i = 0; spec_i < spec_list.size(); ++spec_i) {           \
...
#  define MRN_KEY_PART_SPEC_LIST_EACH_END()                                    \
    ...
    while (false)
#else
#  define MRN_KEY_PART_SPEC_LIST_N_ELEMENTS(spec_list) spec_list.elements
#  define MRN_KEY_PART_SPEC_LIST_EACH_BEGIN(spec_list, spec)                   \
    do {                                                                       \
...
#if MYSQL_VERSION_ID >= 80021 && !defined(MRN_MARIADB_P)
#  define MRN_FIELD_IS_UNSIGNED(field) ((field)->is_unsigned())
#  define MRN_FIELD_FIELD_INDEX(field) ((field)->field_index())
#  define MRN_FIELD_ALL_FLAGS(field)   ((field)->all_flags())
#  define MRN_FIELD_FIELD_PTR(field)   ((field)->field_ptr())
#  define MRN_FIELD_SET_FIELD_PTR(field, new_ptr)                              \
    ((field)->set_field_ptr((new_ptr)))
#else
#  define MRN_FIELD_IS_UNSIGNED(field)            ((field)->unsigned_flag)
#  define MRN_FIELD_FIELD_INDEX(field)            ((field)->field_index)
#  define MRN_FIELD_ALL_FLAGS(field)              ((field)->flags)
#  define MRN_FIELD_FIELD_PTR(field)              ((field)->ptr)
#  define MRN_FIELD_SET_FIELD_PTR(field, new_ptr) (field)->ptr = (new_ptr)
#endif
...
#else
#  define MRN_TABLE_RESET(table) (memset((table), 0, sizeof(TABLE)))
...
#  define MRN_GET_DB_NAME(table_list) (table_list->get_db_name().str)
#else
#  define MRN_GET_TABLE_NAME(query_tables) (query_tables->get_table_name())
#  define MRN_GET_TABLE_NAME_LENGTH(query_tables)                              \
    (strlen(query_tables->get_table_name()))
#  define MRN_GET_DB_NAME(table_list) (table_list->get_db_name())
#endif
...
#  define MRN_HA_EXTRA_ABORT_COPY_NAME "HA_EXTRA_ABORT_COPY"
#  define MRN_HA_EXTRA_ABORT_COPY      HA_EXTRA_ABORT_COPY
#else
